...
template <typename T> struct SIMULATOR {
  ...
  void event_create(double now) {
    ...
    if (cpu.cores > 0) {
      ...
      eq.push(Event<Task>(now + task->front_duration() + cpu.context_switch,
                          task, Event<Task>::Type::cpu_done, 0,
                          cpu.utilization));
      cpu.cores--;
    } else {
      rq.push(task);
  ...
  void do_cpu_task(std::shared_ptr<Task> task, double now) {
    if (cpu.cores > 0) {
      ...
      // Get memory. If get_page returns false
      // a miss occured
      if (!cache.get_page(task->front_page_number()))
        now += MISS_PENALTY;
      eq.push(Event<Task>(now + task->front_duration() + cpu.context_switch,
                          task, Event<Task>::Type::cpu_done, 0,
                          cpu.utilization));
  ...
  // Do the simulation
  void doSIM(int cores, double context_switch, int number_io, int number_task,
             int t_no) {
    trial_no = t_no;
